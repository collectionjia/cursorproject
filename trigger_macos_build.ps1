...
Invoke-RestMethod `
    -Uri "https://api.github.com/repos/$Owner/$Repo/actions/workflows/build-macos.yml/dispatches" `
    -Headers $headers -Method Post -Body $body -ContentType "application/json"

Write-Host "Triggered. Open: https://github.com/$Owner/$Repo/actions"
if (-not $IncludeIntel) {
    Write-Host "arm64 only — Intel job skipped (use -IncludeIntel when arm64 succeeds)."
}
